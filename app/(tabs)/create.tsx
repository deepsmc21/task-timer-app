...
function parseTimeOnDate(baseDate: Date, hhmm: string): Date | null {
...
export default function CreateTask() {
    ...
    async function onSave() {
        if (!user) {
            router.replace("/login"); // falls du umstrukturiert hast, sonst "/(tabs)/login"
            return;
        }

        const startAt = parseTimeOnDate(date, startHHMM);
        let endAt = parseTimeOnDate(date, endHHMM);
        ...
        // Wenn Ende <= Start, dann Ende automatisch am nächsten Tag
        if (endAt <= startAt) {
    ...
    return (
        <View style={{ flex: 1, padding: 16, gap: 12 }}>
            ...
            <TextInput
                ...
                onChangeText={setPlannedMinutes}
                style={{ borderWidth: 1, padding: 10, borderRadius: 8 }}
            />
            ...
            >
                <Text style={{ textAlign: "center" }}>Datum: {formatDateYYYYMMDD(date)} (ändern)</Text>
            </Pressable>
            ...
            <Pressable onPress={onAddLocation} style={{ padding: 12, borderRadius: 10, borderWidth: 1 }}>
                <Text style={{ textAlign: "center" }}>
                    {location ? "Location gesetzt ✅" : "Location hinzufügen (GPS)"}
                </Text>
            </Pressable>
